// DO NOT EDIT THIS FILE
// IT WILL BE RESET TO DEAFULT BEFORE DEPLOYMENT

let hash = "00000000000000000000000000000000";
if (randomHashForTesting) hash = getRandomHash();

let random1ofx;

const queryString = window.location.search;
console.log(queryString);
if (queryString) {
    getParams(queryString);
}

seedRandomness();

function getRandomHash() {
    var result = "";
    var characters =
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
    var charactersLength = characters.length;
    for (var i = 0; i < 32; i++) {
        result += characters.charAt(
            Math.floor(Math.random() * charactersLength)
        );
    }
    return result;
}

function getParam(urlParams, p) {
    return urlParams.get(p);
}

function getParams(queryString) {
    const urlParams = new URLSearchParams(queryString);
    hash = getParam(urlParams, "hash");
}
window.addEventListener("message", (e) => {
    var data = e.data;
    if (data.hasOwnProperty("1ofxQueryString")) {
        getParams(data["1ofxQueryString"]);
        triggerDraw();
    }
});

function triggerDraw() {
    seedRandomness();
    redraw();
}

function windowResized() {
    seedRandomness();
    resetAnimation = true;
    windowResizedUser();
}

function getRNG(hash) {
    return sfc32(...cyrb128(hash));
}

function seedRandomness() {
    random1ofx = getRNG(hash);
}

function cyrb128(str) {
    let h1 = 1779033703,
        h2 = 3144134277,
        h3 = 1013904242,
        h4 = 2773480762;
    for (let i = 0, k; i < str.length; i++) {
        k = str.charCodeAt(i);
        h1 = h2 ^ Math.imul(h1 ^ k, 597399067);
        h2 = h3 ^ Math.imul(h2 ^ k, 2869860233);
        h3 = h4 ^ Math.imul(h3 ^ k, 951274213);
        h4 = h1 ^ Math.imul(h4 ^ k, 2716044179);
    }
    h1 = Math.imul(h3 ^ (h1 >>> 18), 597399067);
    h2 = Math.imul(h4 ^ (h2 >>> 22), 2869860233);
    h3 = Math.imul(h1 ^ (h3 >>> 17), 951274213);
    h4 = Math.imul(h2 ^ (h4 >>> 19), 2716044179);
    return [
        (h1 ^ h2 ^ h3 ^ h4) >>> 0,
        (h2 ^ h1) >>> 0,
        (h3 ^ h1) >>> 0,
        (h4 ^ h1) >>> 0,
    ];
}

function sfc32(a, b, c, d) {
    return function () {
        a >>>= 0;
        b >>>= 0;
        c >>>= 0;
        d >>>= 0;
        var t = (a + b) | 0;
        a = b ^ (b >>> 9);
        b = (c + (c << 3)) | 0;
        c = (c << 21) | (c >>> 11);
        d = (d + 1) | 0;
        t = (t + d) | 0;
        c = (c + t) | 0;
        return (t >>> 0) / 4294967296;
    };
}
